...
**Baseline mapping:** PR-01, PR-04. **Dependencies:** [DF-08](08-reference-pptx-writer.md), [DF-20](20-windows-native-office-worker.md)
...
### DF-09.R07 — Excel-backed chart edit probe

For each required chart fixture, use ChartData.Activate before Workbook access, inspect embedded data, edit a declared value on a probe copy and save/reopen. Verify workbook cells, chart cache and refreshed visual output agree; detect any external link. See W06/W11/W12 in WINDOWS_SOURCES.

### DF-09.R08 — Native diagram and table behavior

Use PowerPoint to inspect cells/merges, group membership and real connector endpoints. Move a node and save/reopen on a copy. Native-line and anchored capabilities remain distinct even with Office installed.
...
- [ ] **DF-09.T06 — Implement feature probes.** Register chart/table/group/connector checks through DF-20 with owned Excel workbook/process cleanup; never terminate a consultant workbook to make a test pass.
...
|---|---|---|---|
| DF-09.AC08 | Chart data edit | Change one synthetic embedded cell and reopen the probe deck. | The new value and chart update persist; other cells stay equal and the original is unchanged. |
| DF-09.AC09 | Excel already in use | An unrelated workbook is open before a chart-data probe. | Worker declines or requests a clean profile; unrelated Excel state is untouched. |
